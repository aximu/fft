...
static void init_tables(void) {
    for (int i = 0; i < FFT_SIZE; i++) {
        double phase = 2.0 * PI * i / FFT_SIZE;
...
int main(int argc, char* argv[]) {
 ...
 //       d_din_i[i] = (rand() % (range)-range / 2);
    ...
    clock_t tv_d_dft_start, tv_d_dft_end, tv_d_dft;
    clock_t tv_d_fft_start, tv_d_fft_end, tv_d_fft;
    clock_t tv_f_fft_start, tv_f_fft_end, tv_f_fft;
    clock_t tv_i_fft_start, tv_i_fft_end, tv_i_fft;

    tv_d_dft_start = clock();
    d_dft(d_din_r, d_din_i, d_dout_r_ref, d_dout_i_ref, FFT_SIZE, 1);
    tv_d_dft_end = clock();

    tv_d_fft_start = clock();
    d_fft(d_din_r, d_din_i, d_dout_r, d_dout_i, FFT_SIZE);
    tv_d_fft_end = clock();

    tv_f_fft_start = clock();
    f_fft(f_din_r, f_din_i, f_dout_r, f_dout_i, FFT_SIZE);
    tv_f_fft_end = clock();

    tv_i_fft_start = clock();
    ...
    printf("Double DFT            %f secs\n", (double)(tv_d_dft_end- tv_d_dft_start)/ CLOCKS_PER_SEC);

    printf("Double FFT            %f secs\n", (double)(tv_d_fft_end - tv_d_fft_start) / CLOCKS_PER_SEC);

    printf("Float FFT            %f secs\n", (double)(tv_f_fft_end - tv_f_fft_start) / CLOCKS_PER_SEC);

    printf("Integer FFT            %f secs\n", (double)(tv_i_fft_end - tv_i_fft_start) / CLOCKS_PER_SEC);
}